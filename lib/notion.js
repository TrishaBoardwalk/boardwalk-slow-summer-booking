...
export const ACTIVITIES = {
  "Floating Ocean Sound Bath": {
    ...
    // One-off schedule overrides — "original ISO" -> "new ISO"
    // Use when a specific session date must move (instructor unavailable, etc.).
    dateOverrides: {
      "2026-06-25": "2026-06-26"  // Thu Jun 25 -> Fri Jun 26
    }
  },
  "Golden Hour": {
    time: "6:00 PM", capacity: 8, dayOfWeek: 0, months: [5, 6], year: 2026, location: "Boardwalk Garden",
    endDate: "2026-07-05"   // last session; Hammock Cocooning takes over the Sunday slot
  },
  "Hammock Cocooning": {
    time: "6:00 PM", capacity: 6, dayOfWeek: 0, months: [6, 7], year: 2026, location: "Boardwalk Garden",
    startDate: "2026-07-12" // first session; Golden Hour ran Sundays before this
  },
  "Garden Tour": {
...
export function getSessionsFor(activityName) {
  ...
  for (const m of cfg.months) {
    ...
    for (let day = 1; day <= last; day++) {
      ...
      if (d.getDay() === cfg.dayOfWeek) {
        const iso = isoDate(d);
        const finalIso = overrides[iso] || iso; // apply override if any
        // Apply optional start/end window
        if (startDate && finalIso < startDate) continue;
        if (endDate   && finalIso > endDate)   continue;
        out.push(finalIso);
      }
    }
  }
...
// ----------------------------------------------------------------------------
// Hours-until-session helper (Aruba time, UTC-4, no DST).
// Returns positive hours if session is in the future, negative if past.
// ----------------------------------------------------------------------------
export function hoursUntilSession(activityName, sessionIso) {
  const cfg = ACTIVITIES[activityName];
  if (!cfg || !sessionIso) return Infinity;

  // Parse "6:00 PM" / "9:00 AM" into 24h hours/minutes
  const m = /^(\d{1,2}):(\d{2})\s*(AM|PM)$/i.exec(cfg.time.trim());
  let hh = 0, mm = 0;
  if (m) {
  ...
  // Aruba is UTC-4 year-round. Session in local Aruba time = UTC + 4.
  const [y, mo, d] = sessionIso.split("-").map(Number);
  const sessionUtcMs = Date.UTC(y, mo - 1, d, hh + 4, mm, 0);
  return (sessionUtcMs - Date.now()) / (1000 * 60 * 60);
}

// ----------------------------------------------------------------------------
// Promote the next waitlisted guest for a specific session to Offered.
// Called from /api/cancel so the waitlist moves within seconds rather than
// waiting for the 15-min cron sweep. Returns the promoted booking (or null).
//
// Guard: if the session is less than 2 hours away, skip promotion entirely —
// the offered guest wouldn't have time to confirm before the session starts.
// Better to let the spot go than dangle an unusable offer.
// ----------------------------------------------------------------------------
export async function promoteNextWaitlistForSession(activityName, sessionIso) {
  const cfg = ACTIVITIES[activityName];
  if (!cfg) return null;

  // Cancel-too-late guard: don't promote if the session is < 2 hours away
  // (or already past). The waitlist offer window is 2 hours, so anything
  // tighter than that can't be acted on.
  const hoursLeft = hoursUntilSession(activityName, sessionIso);
  if (hoursLeft < 2) return null;

  const rows = await queryBookingsBySession(activityName, sessionIso);
  const confirmed = rows.filter(r => r.status === "Confirmed").reduce((s, r) => s + r.partySize, 0);
  const spacesLeft = cfg.capacity - confirmed;
  const currentlyOffered = rows.filter(r => r.status === "Offered");
  const waitlist = rows.filter(r => r.status === "Waitlisted")
    .sort((a, b) => (a.waitlistPosition ?? 999) - (b.waitlistPosition ?? 999));

  if (spacesLeft <= 0) return null;
  if (currentlyOffered.length > 0) return null;
